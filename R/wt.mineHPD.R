...
#' current options.
#'
#' @param categories A vector giving the categories that should go on different 
#' axes. This parameter was added by Nathan De Fruyt
#'
...
wt.mineHPD <- function(HPD, option = "rad <- tot.edge.count", categories = c()) {
  ...
  ## new option created by Nathan De Fruyt
  ## this option allows for assigning different types of nodes to different axes.
  
  for (n in 1:nn) {
    pat <- paste("\\b", nodes$lab[n], "\\b", sep = "")
    
    for (i in 1:length(categories)){
      if (pat %in% categories[i]){
        nodes$axis[n] <- i
      }
    }
  }
  ...
  if (option == "remove zero edge") {
    ...
    HPD <- wt.mineHPD(HPD, option = "remove self edge")
    HPD <- wt.mineHPD(HPD, option = "remove virtual edge")
    return(HPD)
